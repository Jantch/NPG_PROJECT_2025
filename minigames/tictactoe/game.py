...
# dla opóźnienia i zamknięcia po zakończeniu gry
POST_GAME_DELAY = 3000  # ms = 3 sekundy
game_over_time  = None
...
def check_winner():
    global winner, draw_flag, game_over, game_over_time
    # wiersze i kolumny
    for i in range(3):
    ...
    if not game_over:
        if board[0][0] == board[1][1] == board[2][2] != None:
            ...
            game_over = True
    # remis
    if not game_over and all(board[r][c] is not None for r in range(3) for c in range(3)):
        draw_flag = True
        game_over = True

    # ustaw czas zakończenia gry, jeśli dopiero co się zakończyła
    if game_over and game_over_time is None:
...
while running:
    ...
    if game_over:
        ...
        # po wyświetleniu komunikatu poczekaj i zamknij okno
        if game_over_time is not None:
            if pygame.time.get_ticks() - game_over_time >= POST_GAME_DELAY:
                pygame.quit()
                sys.exit(result)
